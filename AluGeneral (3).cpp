...
void AluGeneral::ProgFU(int MK, LoadPoint Load)
{
	...
	case 112: //RangeProgExec Выполнить программу для цикла range
	{
		...
		{ // Реализация цикла
			ThreadStack.back().MkStage = 0;
			int delta = abs(ThreadStack.back().AluStack.back().accumulator.ToInt() - ThreadStack.back().AluStack.back().accumulator.ToInt()) + 1;
			for (int i = ThreadStack.back().AluStack.back().accumulator.ToInt(); abs(ThreadStack.back().AluStack.back().accumulator.ToInt() - i) - delta > 0; i += ThreadStack.back().RangeStep)
			{
				delta = abs(ThreadStack.back().AluStack.back().accumulator.ToInt() - i);
				ProgExec(Load.Point);
			}
		}
		break;
