...
void	ALU::add(LoadPoint Load) // Сложение
{
	if (Load.Point == nullptr) {
		//ProgExec(NoOperandErrProg); // Запуск подпрограммы ошибки "Нет операнда"
		return;
	}
	if (Load.Type >> 1 == Dstring)
	{
		...
		//   ProgExec(UncompatableTypesErrProg);
		return;
	}
	 else if (Load.isDigitBool() && Load.isDigitBool(Stack.back().accumType))
	{
		...
		if (Stack.back().accumType >> 1 == Dbool && Load.Type >> 1 == Dbool)
		 ...
		 // ProgExec(((Threader*)Parent)->UncompatableTypesErrProg);
		return;
	}
}
...
{
	if (Load.Point == nullptr) {
		// ProgExec(((Threader*)Parent)->NoOperandErrProg); // Запуск подпрограммы ошибки "Нет операнда"
		return;
	}
